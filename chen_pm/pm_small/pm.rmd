...
```{r}
set.seed(0)
train_index <- sample(1:nrow(pm_cov), 400, replace = FALSE)
```


```{r}

x_tr <- as.matrix(cbind(long[train_index], lat[train_index]))
x_te <- as.matrix(cbind(long[-train_index], lat[-train_index]))
...
mod_train_dnn <- model_dnn %>%
  fit(x = x_tr, y = pm_tr, epochs = 100, batch_size = 16, 
...
loss_dnn <- evaluate(model_dnn, x_te, pm_te)[2]
...
x_tr <- as.matrix(cbind(cov_tr, basis_tr))
x_te <- as.matrix(cbind(cov_te, basis_te))
...
loss_dk <- evaluate(model_dk, x_te, pm_te)[2]
...
model_ck %>% load_model_weights_hdf5("C:/Users/10616/Desktop/temp/best_weights.h5")
 ...
 loss_ck <- evaluate(model_ck, list(basis_te_1,basis_te_2,basis_te_3,cov_te), pm[-train_index])
```
