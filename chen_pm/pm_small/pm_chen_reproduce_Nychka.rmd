...
test_idx <- 1
ggplot() +
  geom_raster(aes(x = basis_1[,1], y = basis_1[,2], fill = as.vector(basis_arr_1[test_idx,,]))) +
  geom_point(aes(x = scaled_long[test_idx], y = scaled_lat[test_idx]))
...
for (i in 1:length(pm)) {
  basis_arr_2[i,,] <- matrix(basis_fun_2[i,], nrow = shape_row_2, ncol = shape_col_2, byrow = T)
}
...
for (i in 1:length(pm)) {
  basis_arr_3[i,,] <- matrix(basis_fun_3[i,], nrow = shape_row_3, ncol = shape_col_3, byrow = T)
}
...
for (i in 1:length(pm)) {
  basis_arr_4[i,,] <- matrix(basis_fun_4[i,], nrow = shape_row_4, ncol = shape_col_4, byrow = T)
}
