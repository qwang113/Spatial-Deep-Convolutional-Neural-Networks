...
p.res.pred$summary.linear.predictor[index.pred,"mean"]
pred_mse <- mean((p.res.pred$summary.linear.predictor[index.pred,"mean"] - soc_te)^2)
...
loss_all <- as.data.frame(cbind(loss_nngp, loss_dnn, loss_dk, loss_ck))
ggplot() +
