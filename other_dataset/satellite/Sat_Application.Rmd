...
calculate_dist_true <- function(x, true_x){
  ...
  return( mean(spDists(matrix(x, ncol = 1), matrix(true_x, ncol = 1))) )
  
}

crps <- function(posterior_matrix, true_value){
  
  between_sample <- apply(posterior_matrix, 1, calculate_dist_between)
  sample_residual <-  apply(posterior_matrix, 1, calculate_dist_true, true_x = true_value)
...
pred_dnn <- as.matrix(read.csv(here::here("D:/77/UCSC/study/Reasearch/temp/dnn_pred.csv")))
pred_dk <- as.matrix(read.csv(here::here("D:/77/UCSC/study/Reasearch/temp/dk_pred.csv")))
pred_ck <- as.matrix(read.csv(here::here("D:/77/UCSC/study/Reasearch/temp/ck_pred.csv")))
pred_inla <- as.matrix(read.csv(here::here("D:/77/UCSC/study/Reasearch/temp/inla_pred.csv")))
