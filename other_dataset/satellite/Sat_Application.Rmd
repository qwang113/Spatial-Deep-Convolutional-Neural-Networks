...
for (j in 1:num_sample) {
  ...
  grid_pred_dk[,j] <- predict(model_dk, grid_te)
}
...
model_ck %>% load_model_weights_hdf5("D:/77/research/temp/best_weights.h5")
 ...
 for (j in 1:num_sample) {
  ...
  grid_pred_ck[,j] <- predict(model_ck, list(grid_basis_te_1,grid_basis_te_2,grid_basis_te_3,grid_cov_te))
}
...
```{r}
# Prediction surface and uncertainty surface
library(FRK)
library(spNNGP)
library(ggplot2)
library(maps)
...
grid_pred_dnn <- read.csv("D:/77/Reasearch/temp/grid_pred_dnn.csv")
grid_pred_dk <- read.csv("D:/77/Reasearch/temp/grid_pred_dk.csv")
grid_pred_ck <- read.csv("D:/77/Reasearch/temp/grid_pred_ck.csv")
grid_pred_inla <- read.csv("D:/77/Reasearch/temp/grid_pred_inla.csv")
...
ggplot() +
  geom_raster(aes(x = grid_long, y = grid_lat, fill = apply(grid_pred_dnn, 1, mean) )) +
  scale_fill_viridis()

ggplot() +
  geom_raster(aes(x = grid_long, y = grid_lat, fill = apply(grid_pred_dk, 1, mean) )) +
  scale_fill_viridis()

ggplot() +
  geom_raster(aes(x = grid_long, y = grid_lat, fill = apply(grid_pred_ck, 1, mean) )) +
  scale_fill_viridis()

ggplot() +
  geom_raster(aes(x = grid_long, y = grid_lat, fill = apply(grid_pred_inla, 1, mean) )) +
  scale_fill_viridis(limits = c(20,55))

#uncertainty

ggplot() +
  geom_raster(aes(x = grid_long, y = grid_lat, fill = apply(grid_pred_dnn, 1, sd) )) +
  scale_fill_viridis()

ggplot() +
  geom_raster(aes(x = grid_long, y = grid_lat, fill = apply(grid_pred_dk, 1, sd) )) +
  scale_fill_viridis()

ggplot() +
  geom_raster(aes(x = grid_long, y = grid_lat, fill = apply(grid_pred_ck, 1, sd) )) +
  scale_fill_viridis()

ggplot() +
  geom_raster(aes(x = grid_long, y = grid_lat, fill = apply(grid_pred_inla, 1, sd) )) +
