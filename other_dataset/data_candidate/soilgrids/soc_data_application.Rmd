...
soc_data <- read.asciigrid(here::here("other_dataset/data_candidate/soilgrids/sl1_som3035_arab3_1000m.asc"))
r <- raster(here::here("other_dataset/data_candidate/soilgrids/sl1_som3035_arab3_1000m.asc"))
ext <- extent(r)
lat_min <- ext[3]
lat_max <- ext[4]
lon_min <- ext[1]
...
nrows <- nrow(r)
ncols <- ncol(r)
lat <- seq(lat_min, lat_max, length.out = nrows)
lon <- seq(lon_min, lon_max, length.out = ncols)
coords <- expand.grid(lon, lat)
data_all <- na.omit(cbind(coords, as.vector(r)))
...
ggplot() +
  geom_boxplot(data = reshape2::melt(loss_all), aes(x = variable, y = value)) +
   coord_flip()
write.csv(as.data.frame(loss_all),here::here("other_dataset/mra_reproduce/loss_mra.csv"),row.names = FALSE)
loss_all <- read.csv(here::here("other_dataset/mra_reproduce/loss_mra.csv"))
