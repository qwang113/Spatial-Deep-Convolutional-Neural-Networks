...
# Create basis function for DK
  ...
  basis_dist_1 <- spDists(cbind(s,1), cbind(basis_1,1))
  basis_dist_2 <- spDists(cbind(s,1), cbind(basis_2,1))
  basis_dist_3 <- spDists(cbind(s,1), cbind(basis_3,1))
  basis_dist_4 <- spDists(cbind(s,1), cbind(basis_4,1))
...
for (curr_index in 1:10) {
  ...
  train_x <- sim_dat[rep_idx,train_index]
  test_x <- sim_dat[rep_idx, -train_index]
  ...
  krig_mean_all[-train_index] <- 1 + exp_sig_21 %*% solve(exp_sig_11) %*% matrix( as.numeric(train_x - 1), ncol = 1 )
  ...
  x_dk <- cbind(1, matrix(as.numeric(sim_dat[rep_idx,]), ncol = 1), basis_fun_all)
