...
for (curr_index in 1:10) {
  ...
  krig_mean_all[-train_index] <- -0.1168 + exp_sig_21 %*% solve(exp_sig_11) %*% matrix( as.numeric(train_y + 0.1168), ncol = 1 )
...
mod_train_dnn <- model_dnn %>%
  fit(x = x_tr, y = y_tr, epochs = 30, batch_size = 32)
...
mod_train_dk <- model_dk %>%
  ...
  mse_vec_dkrig[curr_index] <- evaluate(model_dk, x_te, y_te)[2]
...
mod_train_ck <- model_ck %>%
  fit(x = x_tr, y = train_y, epochs = 30, batch_size = 128)
