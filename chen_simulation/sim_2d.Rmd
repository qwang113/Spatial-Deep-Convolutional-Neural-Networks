...
```{r}
sim_size = 50
sim_coords <- expand.grid(seq(0,1,length.out = sim_size),seq(0,1,length.out = 50))
...
mod_train_dk <- model_dk %>%
  fit(x = x_tr, y = train_y, epochs = 500, batch_size = 64, callbacks = list(model_checkpoint), validation_data = list(x_te, test_y))
