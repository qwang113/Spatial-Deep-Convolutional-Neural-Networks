...
for (curr_index in 1:10) {
  ...
  print(paste("Now doing iteration ", curr_index))
...
# dnn_mean_all 
  ...
  # nn_mean_curr <- predict(model_dnn, x_te)
  # mse_vec_nn_curr <- evaluate(model_dnn, x_te, y_te)[2]
  # model_dnn <- load_model_weights_hdf5()
  nn_mean_all[-train_index] <- predict(model_dnn, x_te)
  mse_vec_nn[curr_index] <- evaluate(model_dnn, x_te, y_te)[2]
  # mse_epoch[curr_epoch] <- mse_vec_nn_curr
  # 
  # epoch_pred[curr_epoch,] <- evaluate(model_dnn, x_te, y_te)[2] 
  # 
  # curr_best_epoch <- which.min(mse_epoch)
  # mse_vec_nn[curr_index] <- mini(mse_epoch)
  # nn_mean_all[-train_index] <- epoch_pred[curr_best_epoch,]
}
...
mod_train_ck <- model_ck %>%
  fit(x = x_tr, y = train_y, epochs = 200, batch_size = 64, callbacks = list(model_checkpoint), validation_data = list(x_te, test_y))
  model_ck %>% load_model_weights_hdf5("best_weights.h5")
...
cowplot::plot_grid(p_obs, p_krig, p_dnn, p_dk, p_ck)
...
```{r}
ggplot() +
  geom_boxplot(data = reshape2::melt(sqrt(mse_mat)), aes(x = variable, y = value)) +
   coord_flip()
```
