...
us_map <- map_data("usa")
color_plate <- c("red","blue","yellow")
# Plot circles with different colors
p <- ggplot() +
  theme_minimal() +
  xlab("Longitude") +
  ylab("Latitude") +
  ggtitle("Basis Function Illustration Havng Three Resolutions") + 
  theme(plot.title = element_text(hjust = 0.5))

for (i in 1:nrow(basis_use)) {
  circle_data <- create_circle(basis_use$loc1[i], basis_use$loc2[i], basis_use$scale[i]*0.6)
  p <- p + geom_polygon(data = circle_data, aes(x, y), fill = alpha(color_plate[basis_use$res[i]], 0.5))
}
p <- p + guides(fill = guide_legend(
  title = "Legend",
  override.aes = list(color = color_plate, fill = color_plate),
  label = c("Res 1", "Res 2", "Res 3")
)) + geom_polygon(data = us_map, aes(x = long, y = lat, group = group), fill = NA, color = "black", linewidth = 1.5) 
print(p)
```
...
write.csv(as.data.frame(pred_inla_g),"D:/77/Research/temp/soc_pred/inla_pred_soc_g.csv",row.names = FALSE)
...
write.csv(crps_dnn_all,"D:/77/Research/temp/soc_pred/crps_dnn_soc.csv",row.names = FALSE)
write.csv(crps_dk_all,"D:/77/Research/temp/soc_pred/crps_dk_soc.csv",row.names = FALSE)
write.csv(crps_ck_all,"D:/77/Research/temp/soc_pred/crps_ck_soc.csv",row.names = FALSE)
write.csv(crps_inla_all,"D:/77/Research/temp/soc_pred/crps_inla_soc.csv",row.names = FALSE)

crps_dnn_all <- as.matrix(read.csv("D:/77/Research/temp/soc_pred/crps_dnn_soc.csv"))
crps_dk_all <- as.matrix(read.csv("D:/77/Research/temp/soc_pred/crps_dk_soc.csv"))
crps_ck_all <- as.matrix(read.csv("D:/77/Research/temp/soc_pred/crps_ck_soc.csv"))
crps_inla_all <- as.matrix(read.csv("D:/77/Research/temp/soc_pred/crps_inla_soc.csv"))
...
write.csv(int_score_dnn,"D:/77/Research/temp/soc_pred/int_dnn_soc.csv",row.names = FALSE)
write.csv(int_score_dk,"D:/77/Research/temp/soc_pred/int_dk_soc.csv",row.names = FALSE)
write.csv(int_score_ck,"D:/77/Research/temp/soc_pred/int_ck_soc.csv",row.names = FALSE)
write.csv(int_score_inla,"D:/77/Research/temp/soc_pred/int_inla_soc.csv",row.names = FALSE)
