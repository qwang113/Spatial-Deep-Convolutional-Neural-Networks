...
for (i in 1:length(pm)) {
  basis_arr_1[i,,] <- matrix(basis_fun_1[i,], nrow = shape_row_1, ncol = shape_col_1)
}
...
for (i in 1:length(pm)) {
  basis_arr_2[i,,] <- matrix(basis_fun_2[i,], nrow = shape_row_2, ncol = shape_col_2)
}
...
for (i in 1:length(pm)) {
  basis_arr_3[i,,] <- matrix(basis_fun_3[i,], nrow = shape_row_3, ncol = shape_col_3)
}
...
```{r}
loss_all <- read.csv(here::here("chen_pm/pm_small/loss_Nychka.csv"))
ggplot() +
  geom_boxplot(data = reshape2::melt(loss_all), aes(x = variable, y = value)) +
   coord_flip()
```
