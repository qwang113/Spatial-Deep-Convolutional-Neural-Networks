...
for (curr_index in 1:num_fold) {
  ...
  curr_res <- likfit(coords = train_coords, data = train_y, ini.cov.pars = c( 100,100), fix.kappa = FALSE, nugget = 0, fix.nugget = TRUE)
...
pred_all <- read.csv(here::here("Qi_simulation/eh_prediction_2d.csv"))
...
krig_mean_all <- as.matrix(pred_all[,1])
nn_mean_all <- as.matrix(pred_all[,2])
dkrig_mean_all <- as.matrix(pred_all[,3])
ckrig_mean_all <- as.matrix(pred_all[,4])
...
cowplot::plot_grid(diff_krig, diff_dnn, diff_dk, diff_ck)
...
```{r}
ggplot() +
  geom_boxplot(data = reshape2::melt(mse_all), aes(x = variable, y = value)) +
   coord_flip()
```
