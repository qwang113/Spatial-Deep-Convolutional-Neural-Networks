...
for (curr_index in 1:num_fold) {
  ...
  x_tr_va <- cbind(as.matrix(cbind(min_max_scale(long),min_max_scale(lat)))[-train_index,],basis_te_1, basis_te_2, basis_te_3)
...
inla_mse <- mean((apply(pred_inla, 1, mean) - y)^2)
print(inla_mse)
...
```{r}
write.csv(as.data.frame(pred_dnn),"D:/77/Research/temp/eh_pred/dnn_pred_eh.csv",row.names = FALSE)
write.csv(as.data.frame(pred_dk),"D:/77/Research/temp/eh_pred/dk_pred_eh.csv",row.names = FALSE)
write.csv(as.data.frame(pred_ck),"D:/77/Research/temp/eh_pred/ck_pred_eh.csv",row.names = FALSE)
write.csv(as.data.frame(pred_inla),"D:/77/Research/temp/eh_pred/inla_pred_eh.csv",row.names = FALSE)
```
...
long <- expand.grid(long_grid, lat_grid)[,1]
lat <- expand.grid(long_grid, lat_grid)[,2]
dnn_mse <- mean((apply(pred_dnn, 1, mean) - y)^2)
dk_mse <- mean((apply(pred_dk, 1, mean) - y)^2)
ck_mse <- mean((apply(pred_ck, 1, mean) - y)^2)
inla_mse <- mean((apply(pred_inla, 1, mean) - y)^2)
...
write.csv(crps_dnn_all,"D:/77/Research/temp/eh_pred/crps_dnn_eh.csv",row.names = FALSE)
write.csv(crps_dk_all,"D:/77/Research/temp/eh_pred/crps_dk_eh.csv",row.names = FALSE)
write.csv(crps_ck_all,"D:/77/Research/temp/eh_pred/crps_ck_eh.csv",row.names = FALSE)
write.csv(crps_inla_all,"D:/77/Research/temp/eh_pred/crps_inla_eh.csv",row.names = FALSE)

crps_dnn_all <- as.matrix(read.csv("D:/77/Research/temp/eh_pred/crps_dnn_eh.csv"))
crps_dk_all <- as.matrix(read.csv("D:/77/Research/temp/eh_pred/crps_dk_eh.csv"))
crps_ck_all <- as.matrix(read.csv("D:/77/Research/temp/eh_pred/crps_ck_eh.csv"))
crps_inla_all <- as.matrix(read.csv("D:/77/Research/temp/eh_pred/crps_inla_eh.csv"))
...
write.csv(int_score_dnn,"D:/77/Research/temp/int_dnn_eh.csv",row.names = FALSE)
write.csv(int_score_dk,"D:/77/Research/temp/int_dk_eh.csv",row.names = FALSE)
write.csv(int_score_ck,"D:/77/Research/temp/int_ck_eh.csv",row.names = FALSE)
write.csv(int_score_inla,"D:/77/Research/temp/int_inla_eh.csv",row.names = FALSE)
...
# facet_zoom() add some layers to the plot
# Add uncertainty surface
# Write all three datasets
is_between <- function(value, lower, upper) {
  return(value >= lower & value <= upper)
}

HPD_dnn_lo <- apply(pred_dnn,1,quantile,0.025)
HPD_dnn_up <- apply(pred_dnn,1,quantile,0.975)
HPD_dk_lo <- apply(pred_dk,1,quantile,0.025)
HPD_dk_up <- apply(pred_dk,1,quantile,0.975)
HPD_ck_lo <- apply(pred_ck,1,quantile,0.025)
HPD_ck_up <- apply(pred_ck,1,quantile,0.975)
HPD_inla_lo <- apply(pred_inla,1,quantile,0.025)
HPD_inla_up <- apply(pred_inla,1,quantile,0.975)

dnn_icr <- sum(is_between(y, HPD_dnn_lo, HPD_dnn_up))/length(y)
dk_icr <- sum(is_between(y, HPD_dk_lo, HPD_dk_up))/length(y)
ck_icr <- sum(is_between(y, HPD_ck_lo, HPD_ck_up))/length(y)
inla_icr <- sum(is_between(y, HPD_inla_lo, HPD_inla_up))/length(y)
```
