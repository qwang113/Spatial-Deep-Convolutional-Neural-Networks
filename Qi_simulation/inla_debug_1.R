...
for (curr_index in 1:num_fold) {
  ...
  long <- min_max_scale(long) * 20
  lat <- min_max_scale(lat) * 20
...
icr_inla_20 <- sum( y>apply(pred_inla, 1,quantile, 0.025) & y<apply(pred_inla, 1,quantile, 0.975) )/length(y)
...
icr_inla_10 <- sum( y>apply(pred_inla, 1,quantile, 0.025) & y<apply(pred_inla, 1,quantile, 0.975) )/length(y)
...
for (curr_index in 1:num_fold) {
  ...
  train_index <- which(train_index_all != curr_index)
  long <- min_max_scale(long) * 5
  lat <- min_max_scale(lat) * 5
  long_tr <- long[train_index]
  lat_tr <- lat[train_index]
  y_tr <- y[train_index]
  coords <- cbind(long_tr, lat_tr)
...
icr_inla_5 <- sum( y>apply(pred_inla, 1,quantile, 0.025) & y<apply(pred_inla, 1,quantile, 0.975) )/length(y)
