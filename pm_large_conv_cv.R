...
pm_large_conv_cv <- function(curr_index, cv_index = cv_index_all, basis_arr = basis_arr_all,  pm_filt = pm_large_all$Arithmetic.Mean,
                             shape_row = row_shape, shape_col = col_shape, img_rows = rows_img, img_cols = cols_img){
  ...
  curr_predict <- predict(model_ck, x_te)
  curr_mse <- (pm_te - curr_predict)^2
  return(curr_mse)
}

# I used parallel before, but this becomes very complicated and time consuming, however, I activated the graphics cards for computing successfully!
# cl <- makeCluster(5)
# clusterEvalQ(cl, {
#   library(FRK)
#   library(spNNGP)
...
# cv_mse_dl_para <- clusterApply(cl = cl, 1:5, fun = pm_large_conv_cv, cv_index = cv_index_all, basis_arr = basis_arr_all, 
#                                pm_filt = pm_large_all$Arithmetic.Mean, shape_row = row_shape, shape_col = col_shape,
#                                img_rows = rows_img, img_cols = cols_img)
# 
# 
# timepara <- system.time( clusterApply(cl = cl, 1:5, fun = pm_large_conv_cv, cv_index = cv_index_all, basis_arr = basis_arr_all, 
#                                       pm_filt = pm_large_all$Arithmetic.Mean, shape_row = row_shape, shape_col = col_shape,
#                                       img_rows = rows_img, img_cols = cols_img))
# stopCluster(cl)
# 
